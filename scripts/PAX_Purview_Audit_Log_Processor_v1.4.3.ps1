# Portable Audit eXporter (PAX) - Purview Audit Log Processor - v1.4.3
<#
.SYNOPSIS
...
.EXECUTIONPOLICY
    No internal execution policy bypass. Use external host invocation if needed:
        powershell.exe -ExecutionPolicy Bypass -File .\PAX_Purview_Audit_Log_Processor_v1.4.3.ps1 -StartDate 2025-10-01 -EndDate 2025-10-02
        pwsh.exe       -ExecutionPolicy Bypass -File .\PAX_Purview_Audit_Log_Processor_v1.4.3.ps1 -StartDate 2025-10-01 -EndDate 2025-10-02

.POWERSHELLVERSIONS
    PS 5.1 & 7+. Parallelization requires PS 7+.

.EXAMPLE
    pwsh -File .\PAX_Purview_Audit_Log_Processor_v1.4.3.ps1 -StartDate 2025-10-01 -EndDate 2025-10-02 -OutputFile C:\Temp\Copilot.csv
.EXAMPLE
    pwsh -File .\PAX_Purview_Audit_Log_Processor_v1.4.3.ps1 -ExplodeArrays -StartDate 2025-10-01 -EndDate 2025-10-02 -OutputFile C:\Temp\Copilot_exploded.csv
.EXAMPLE
    pwsh -File .\PAX_Purview_Audit_Log_Processor_v1.4.3.ps1 -ExplodeDeep -StartDate 2025-10-01 -EndDate 2025-10-02 -OutputFile C:\Temp\Copilot_deep.csv
.EXAMPLE
    powershell -File .\PAX_Purview_Audit_Log_Processor_v1.4.3.ps1 -StartDate 2025-10-01 -EndDate 2025-10-02 -OutputFile C:\Temp\Copilot.csv
.EXAMPLE
    # Offline replay (simple forced explosion) of a previously exported raw CSV
    pwsh -File .\PAX_Purview_Audit_Log_Processor_v1.4.3.ps1 -RAWInputCSV .\output\Copilot_RAW_20251001.csv -OutputFile C:\Temp\Copilot_replay_exploded.csv
.EXAMPLE
    # Offline replay with date & activity filtering + deep flatten
    pwsh -File .\PAX_Purview_Audit_Log_Processor_v1.4.3.ps1 -RAWInputCSV .\output\Copilot_RAW_20251001.csv -ExplodeDeep -StartDate 2025-10-01 -EndDate 2025-10-02 -ActivityTypes CopilotInteraction -OutputFile C:\Temp\Copilot_replay_deep.csv
.EXAMPLE
    # Deep flatten (wide) with higher schema sample & moderate chunk size (balance column coverage vs memory)
    pwsh -File .\PAX_Purview_Audit_Log_Processor_v1.4.3.ps1 -ExplodeDeep -StartDate 2025-10-01 -EndDate 2025-10-02 -StreamingSchemaSample 4000 -StreamingChunkSize 3000 -OutputFile C:\Temp\Copilot_deep_tuned.csv
.EXAMPLE
    # Extremely wide deep flatten: maximize schema sample, reduce chunk size for lower peak memory
    pwsh -File .\PAX_Purview_Audit_Log_Processor_v1.4.3.ps1 -ExplodeDeep -StartDate 2025-10-01 -EndDate 2025-10-02 -StreamingSchemaSample 6000 -StreamingChunkSize 1500 -OutputFile C:\Temp\Copilot_deep_memoryguard.csv
.EXAMPLE
    # Fast header freeze (narrow schema expectation) – smaller sample, larger chunk for throughput (risk: late columns ignored)
    pwsh -File .\PAX_Purview_Audit_Log_Processor_v1.4.3.ps1 -ExplodeDeep -StartDate 2025-10-01 -EndDate 2025-10-02 -StreamingSchemaSample 800 -StreamingChunkSize 6000 -OutputFile C:\Temp\Copilot_deep_fastfreeze.csv
#>
