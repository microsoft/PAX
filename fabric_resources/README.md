...
- **M365 rollup append.** Under `-IncludeM365Usage` + `-Rollup` / `-RollupPlusRaw`, the embedded M365 Bundle processor emits three files into the same destination — `_Rollup.csv`, `_UserStats.csv`, and `_SessionCohort.csv`. `-AppendFile` MUST point to the `_Rollup.csv` leaf; the two sidecars are recomputed each run, anchored off the same leaf stem, and overwrite their derived destination URLs in-place. Sidecar leaves and CopilotInteraction (`_Interactions.csv`) / event-level (`_Exploded.csv`) leaves are rejected at pre-flight.
