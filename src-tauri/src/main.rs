...
async fn export_hardcoded_script(
  ...
  // Check if the output file is a default-pattern filename vs a custom user-specified path
  // Default pattern: ends with "Purview_Export_YYYYMMDD_HHMMSS.csv" (generated by defaultOutputPath())
  let is_default_filename = {
    use regex::Regex;
    let pattern = Regex::new(r"\\Purview_Export_\d{8}_\d{6}\.csv$").unwrap_or_else(|_| Regex::new("").unwrap());
    pattern.is_match(&outputFile) || outputFile.is_empty()
  };
  
  let (output_file_config, csv_output_desc, runtime_note) = if is_default_filename {
    // Default: Generate dynamic filename to avoid file locking issues
    (
      "# Generate dynamic output file name with current timestamp to avoid file locking issues\n$OutputFile = 'Purview_Export_' + (Get-Date -Format 'yyyyMMdd_HHmmss') + '.csv'".to_string(),
      "Dynamic filename (Purview_Export_YYYYMMDD_HHMMSS.csv)".to_string(),
      " (generated at runtime)".to_string()
    )
  } else {
    // Custom: Use the exact path/filename specified by the user
    (
      format!("# User-specified output file path\n$OutputFile = '{}'", outputFile.replace("'", "''")),
      outputFile.clone(),
  ...
  // Track state for proper script processing
  let mut in_param = false;
  let mut param_brace_count = 0;
  let mut skip_show_help = false;
  let mut skip_help_guard = false;
  let mut help_brace_count = 0;
  let mut _guard_brace_count = 0;
  let mut injected_config = false;
  let mut found_requires = false;
  let mut replaced_main_param = false;
  
  for line in src.lines() {
    let trimmed = line.trim();
    
    // Handle #Requires line - keep it at the top
    ...
    // Handle param block - replace ONLY the main script param with simple param
    if !in_param && trimmed.starts_with("param(") && !replaced_main_param {
      in_param = true;
      param_brace_count = 0;
      ...
      // If param closes on same line, inject replacement now
      if param_brace_count == 0 {
        in_param = false;
        replaced_main_param = true;
    ...
    // Continue handling multi-line param block (only for main script param)
    if in_param && !replaced_main_param {
      for ch in line.chars() {
        match ch {
          '(' => param_brace_count += 1,
          ')' => param_brace_count -= 1,
    ...
    // Skip Show-Help function completely
    if !skip_show_help && trimmed.starts_with("function Show-Help") {
      skip_show_help = true;
      help_brace_count = 0;
      // Count braces on the same line
      for ch in line.chars() {
        if ch == '{' { help_brace_count += 1; }
        else if ch == '}' { help_brace_count -= 1; }
      }
      continue;
    }
    if skip_show_help {
      ...
      if help_brace_count <= 0 {
        skip_show_help = false;
      }
      continue;
    }
    
    // Skip help guard completely - look for if statement that contains help checks
    if !skip_help_guard && trimmed.starts_with("if ($Help") {
      skip_help_guard = true;
      _guard_brace_count = 0;
      // Count braces on the same line
      for ch in line.chars() {
        if ch == '{' { _guard_brace_count += 1; }
        else if ch == '}' { _guard_brace_count -= 1; }
      }
      continue;
    }
    if skip_help_guard {
      ...
      // End when we've closed all braces for this if block
      if _guard_brace_count <= 0 {
