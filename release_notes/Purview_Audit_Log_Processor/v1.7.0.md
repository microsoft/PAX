...
Fixed critical PowerShell type ambiguity errors in filtering sections; Fixed variable name collisions between progress tracking and elapsed time calculations; Added explicit [int] type casts to prevent 'ambiguous overloads' errors when using multiple filter switches; Renamed parameters from UserId to UserIds and GroupName to GroupNames to match Microsoft's official naming convention (Search-UnifiedAuditLog); Improved explosion metrics display to clearly show multi-row records, single-row records, and total record counts; Added PromptFilter feature with Both/PromptsOnly/ResponsesOnly options for isolating conversation turns; Added ExcludeAgents switch for filtering out agent interactions; Fixed blank Message_isPrompt values in output; Enhanced filter display to show UserIds and GroupNames in summary section; Fixed Stage 1 and Stage 2 filtering logic for PromptFilter=Both mode
